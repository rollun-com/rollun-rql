...
({
	define: typeof define != "undefined" ? define : function (deps, factory) {
		module.exports = factory(exports, require("./parser"), require("./query"), require("./util/each"), require("./util/contains"));
	}
	...
	exports.operators = {
		...
		not: function (node) {
			...
			var items = this;
			for (var i = 0; i < items.length; i++) {
				var idProperty = "rqlId" + rqlId++;
				var entry = items[i];
				entry.__rqlId = idProperty;
				items[i] = entry;
			}
			...
			for (i = 0; i < items.length; i++) {
				...
				if (notNeededItems.find(function (notNeededItem) {
					return notNeededItem.__rqlId === itemRqlId;
				})) {
					delete items[i];
				} else {
